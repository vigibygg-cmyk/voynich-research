...
# Fixed by V/VI phases: Use languages structurally (agglutination/composition)
# is closest to Voynich manuscript, so test is maximally strict.
HISTORICAL_CONTROLS = [
...
def create_ontology_mappers(train_words, is_voynich=True):
    ...
    if is_voynich:
        ...
        # since they don't have Voynich words and True Ontology wouldn't work there.
    ...
    # FIX: Now we really shuffle word class dependencies!
    shuffled_mapper = {}
    mapped_words = list(true_mapper.keys())
    mapped_cats = list(true_mapper.values())
    random.shuffle(mapped_cats) # Swap true values!
