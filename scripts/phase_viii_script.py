...
def get_visual_category(folio_str):
    """Assigns the page to one of the main manuscript categories (checking all sections)."""
...
def extract_prefix_context(df, prefix, source_name):
    """Extracts lines containing the specified macro-operator (prefix)."""
    matching_lines = []
...
def extract_chain_context(df, chain, source_name):
    """Extracts lines with the exact hard syntactic chain."""
