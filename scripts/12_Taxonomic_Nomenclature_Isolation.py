...
def calculate_dynamic_parameters(df):
    ...
    stop_words = set(w for w, c in word_counts.most_common(20))
...
def calculate_lexeme_tfidf(section_lexemes):
    ...
    threshold = max(3, np.percentile(counts, 90)) if counts else 3
    valid_lexemes = {l for l, c in global_counts.items() if c >= threshold}
