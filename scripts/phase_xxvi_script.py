...
# Morphological dictionary (derived from Phase VII, XIV, and XIX results)
...
def ontological_parse_and_encode(words):
    ...
    Each successfully recognized morphological dictionary token is encoded as 12 bits (1.5 bytes).
    Unrecognized words are encoded in standard 8-bit UTF-8 format.
    """
    total_bits = 0
    parsed_words = 0
    ...
    for word in words:
        ...
        # Greedy Match: trying to find the longest prefix and suffix
        prefix_match = None
        suffix_match = None
        root = word
        ...
        # FIX (LOGIC TRAP): Word encoded using dictionary method only if
        # if a prefix OR suffix was successfully detected in it.
        # Otherwise, the word is considered unrecognized and encoded in UTF-8 format.
        if prefix_match is not None or suffix_match is not None:
            parsed_words += 1
            # Calculation of optimized size
            # 12 bits for each recognized dictionary component (prefix, suffix)
            if prefix_match: total_bits += 12
            if suffix_match: total_bits += 12
            
            # Remaining root (if it exists after splitting) encoded as 12-bit index
            if len(root) > 0:
                total_bits += 12 
                
            total_bits += 8 # Tarpo separatorius (1 baitas)
        else:
            # Standard fallback for completely undecipherable words
